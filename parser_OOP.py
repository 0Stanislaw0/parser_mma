...
class Parser_MMA():
    ...
    def krauler(self, urls):
        ...
        for url in urls:  # краулер
            ...
            for index, div in enumerate(divs):
                ...
                suburl_soup = bs(self.get_html("".join([new, href]), headers),
                                 'lxml')  # открываем статью, new and url  реализовано так, потому ссылка которая содержится в href представлена неполная
                ...
                description = table.find('td', class_='eMessage').get_text().split("$")[
                    0]  # обрезаем. т.к. дальше копируется аякс запрос
    ...
    def write_to_PostGreSQl(self, news):
        connect = psycopg2.connect(
            "host=localhost dbname=postgres user=postgres password=123")  # Подключаемся к postgresql.
        cursor = connect.cursor()  # создаем курсор в этой бд
        for new in news:
            cursor.execute(
                "INSERT INTO posts(headname,encoded_img,url,description) VALUES ( %s, %s, %s, %s)",
                (new['text'], new['encoded_img'], new['href'], new['description']))
        connect.commit()
